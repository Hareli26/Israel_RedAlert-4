...
"""
import sys, os, json, time, math, threading, subprocess, ctypes, winreg, hashlib
...
class Config:
    DEF = {
        ...
        "auto_fullscreen":True,"show_map":True,"autostart":False,
...
class Alert:
    def __init__(self,raw):
        ...
        self.ts=datetime.now(); self.info=CATEGORIES.get(self.cat,DEFAULT_CAT)
    @property
...
class FloatingWidget(QWidget):
    ...
    def _build(self):
        ...
        self._bg=sb("🌍","שיתוף מיקום Google"); self._bx=sb("−","מזעור")
        self._bc.clicked.connect(self.sig_settings); self._bm.clicked.connect(self.sig_map)
        self._bg.clicked.connect(self.sig_google)
...
class FullScreen(QWidget):
    ...
    def __init__(self,history,active=None,timeout=0):
        super().__init__()
...
class RedAlertApp(QApplication):
    ...
    def _close_shelter_scr(self):
        if self._shelter_scr is not None:
            try: self._shelter_scr.close()
            except RuntimeError: pass
            self._shelter_scr = None

    def _on_shelter_ok(self):
        """Called when shelter countdown finishes – show all-clear screen."""
        self._shelter_scr = None
        self._shelter_end = 0.0
        # Play a soft reassuring sound
        if self.config.get("sound", True):
            ...
            _th.Thread(target=_soft, daemon=True).start()
        self._tray.showMessage("✅  מותר לצאת",
            "פיקוד העורף אישר יציאה – ניתן לצאת מהמרחב המוגן.",
            QSystemTrayIcon.Information, 8000)
        self._allclear_scr = AllClearScreen()
        self._allclear_scr.show()

    # ── FullScreen — crash-safe ────────────────────────────────
    def _fullscreen(self):
        if self._fs is not None:
            try:
                if self._fs.isVisible(): self._fs.close()
            except RuntimeError: pass
            self._fs=None
        timeout = self.config.get("fullscreen_timeout", 15)
        self._fs=FullScreen(self.history, self.history[0] if self.history else None,
                            timeout=timeout)
        self._fs.sig_shelter_confirmed.connect(self._on_shelter_button)
        self._fs.destroyed.connect(self._on_fs_destroyed)
        self._fs.show()

    def _on_fs_destroyed(self): self._fs=None
